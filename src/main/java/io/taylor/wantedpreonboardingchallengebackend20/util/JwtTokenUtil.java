...
public class JwtTokenUtil {
    ...
    public AuthenticatedUser getUserIdFromToken(String accessToken) {
        Claims claims = getClaimsFromToken(accessToken);
        if (claims.getExpiration().before(new Date()))
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "유효하지 않은 토큰입니다.");
        return new AuthenticatedUser(claims.get("userId", Long.class), claims.get("email", String.class), claims.get("nickName", String.class));
    }
